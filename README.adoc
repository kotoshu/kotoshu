...
Kotoshu is **v0.3.0** — building on the 0.2 cut, this release adds
integrity verification, a `fetch` pre-warm CLI, SARIF output, an
`--interactive` review loop, and strict-mode resource resolution.

=== What works in 0.3

* `Kotoshu.correct?("hello")` / `Kotoshu.suggest("helo")` — auto-downloads en_US Hunspell on first call
* `Kotoshu.check(text, language: "en", offline: false)` — full document check
...
* `kotoshu check FILE` CLI with these flags:
  ...
  ** `--verbose`
* `kotoshu fetch LANGUAGE [LANGUAGE ...]` — pre-warm spelling + frequency caches for offline use
* Exit codes: `0` clean, `1` errors found, `2` usage error, `3` resource fetch failed
...
NOTE: The semantic (ONNX) path is gated behind the `KOTOSHU_REQUIRE_ONNX=1`
environment variable in this release. Without it, only the traditional
Hunspell path runs.
...
`Kotoshu::Models::OnnxModel` (requires `KOTOSHU_REQUIRE_ONNX=1`).
...
frequency lists, and embedding models. All resources are automatically downloaded
from GitHub repositories and cached locally in `~/.kotoshu/`.
...
* **Cache Path**: `~/.kotoshu/languages/{code}/`
...
* **Cache Path**: `~/.kotoshu/frequency-lists/{code}/`
* **TTL**: 7 days (604,800 seconds)
...
* **Cache Path**: `~/.kotoshu/models/{code}/models/{type}/`
...
Language Cache:
  Directory: /Users/username/.kotoshu/languages
  Resources cached: 2
  Size: 2.45 MB
  Hits: 15, Misses: 2
  Hit rate: 88.2%
...
Frequency Cache:
  Directory: /Users/username/.kotoshu/frequency-lists
  Resources cached: 1
  Size: 815.84 KB
  Hits: 42, Misses: 1
  Hit rate: 97.7%
...
Model Cache:
  Directory: /Users/username/.kotoshu/models
  Resources cached: 0
  Size: 0 B
  Hits: 0, Misses: 0
  Hit rate: 0.0%
...
git clone https://github.com/kotoshu/dictionaries.git ~/.kotoshu/models

# Or use built-in download
kotoshu model download en --type onnx
----

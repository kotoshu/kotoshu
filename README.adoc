...
* `Kotoshu.spellchecker_for(lang)` — per-language checker resolved from the cache (`strict:` is a `Kotoshu.setup` option)
* `kotoshu check FILE` CLI with these flags:
  ** `--language LANG|auto` — any set-up language code, script variants included (`sr-Latn`) (default: `auto`)
...
Kotoshu serves every language staged in the dictionaries manifest —
36 full-feature codes with per-language modules (script-aware
tokenizer, national keyboard grid, engine-verified specimens) and the
script-fallback basic tier for the rest; see the
<<language-matrix, Language Support Matrix>> for the breakdown.
Semantic reranking adds an ONNX model where the models registry pins
one (50+ languages), for example:
...
[[typo-retrieval]]
=== Typo Retrieval (opt-in)

The typo-retrieval layer adds a 0.5 MB character bi-encoder to the
suggestion pipeline: it retrieves the twenty vocabulary entries nearest
a misspelling's character shape, and the language's FULL tier rescores
them exactly. On the frozen real-pair benchmarks it lifts top-5 recall
over the full tier alone (+6.3 points on English, +10 to +29 on de/es/ru).

Setup fetches everything with one flag:

[source,bash]
----
kotoshu setup en --typo
----

The layer arms only when you opt in with `KOTOSHU_TYPO_RETRIEVAL=1`;
without it, suggestions are byte-identical to the base pipeline. A
prebuilt 26 MB retrieval matrix is served by the model registry, so
arming is a download (about half a second) instead of a 25-second
derivation. The matrix rows pair with the language's full-tier
vocabulary, so keep the full tier cached (setup fetches it).
...
Words that are intentionally "wrong" - product names, code identifiers
quoted in prose, dialect - can be suppressed in place instead of being
...
----
Each format recognizes the directive in its own comment syntax, and the
directive line itself is never spellchecked:
[options="header"]
|===
|Format |Form |Trailing directive

|Plain text
|a bare line: `kotoshu:disable-line`
|not supported
...
The Ruby API picks this up automatically:
`Kotoshu.spellchecker_for("en").check(text)` moves suppressed words to
`result.suppressed_errors` (each marked `suppressed: true` and
`suppressed_by: "inline"`) so only real errors stay in `result.errors`.
Pass `--show-suppressed` to `kotoshu check` to list them.

[[ci-baselines]]
== CI Baselines
Freeze existing spelling debt so CI fails only on NEW errors:
[source,bash]
----
# Record current findings: one entry per file+word with a count,
# stable across reformatting
kotoshu baseline init README.md docs/*.adoc

# CI: errors covered by the baseline pass, new errors fail
kotoshu check README.md --baseline .kotoshu-baseline.json
----
A summary line reports baseline entries that have gone stale (the error
no longer exists), so the debt shrinks visibly; refresh by re-running
`kotoshu baseline init`. JSON output adds `suppressedCount` /
`suppressedErrors` plus a `baseline` block, and SARIF marks
baseline-suppressed results with a `suppressions` entry
(`justification: "baseline"`).

[[pre-commit]]
== Git pre-commit Hook
The gem repository ships a https://pre-commit.com[pre-commit] hook
definition (`.pre-commit-hooks.yaml`):
[source,yaml]
----
repos:
  - repo: https://github.com/kotoshu/kotoshu
    rev: v0.7.0   # pin to a released tag
    hooks:
      - id: kotoshu
----
NOTE: The hook uses `language: system`: it runs the `kotoshu` executable
from PATH, so it honestly requires a Ruby environment with the gem
installed (`gem install kotoshu`, then `kotoshu setup en`). A
WebAssembly client (in development) is the future zero-Ruby path.
...
Kotoshu serves languages in tiers. Every language staged in the
kotoshu/dictionaries manifest downloads through `kotoshu setup` and
checks through the Hunspell engine. Region variants fold onto their
base dictionary (`en-GB` resolves `en`, `es-MX` resolves `es`);
script variants keep a dictionary of their own (`sr-Latn` stages the
Latin Serbian dictionary alongside the Cyrillic `sr`).

.Language Support Matrix
[width="100%",cols="^2,^3,^3",options="header"]
|=========================================
| Tier | Languages | What it carries                          |
|=========================================
| Full feature (35 languages, 36 codes) | ar bg ca cs da de el en es et fa fr he hr hu id it ko lt lv nb ne nl nn pl pt ro ru sk sl sr sr-Latn sv tr uk vi | Per-language module: staged dictionary + Hunspell affix rules, script-aware tokenizer, national keyboard grid for typo-proximity suggestions, engine-verified specimens |
| Basic (26 languages) | br cy eo eu fo fur fy ga gd gl hy hyw ia ie is ka la lb ltg mk mn nds oc rw tk tlh | Staged dictionary + script-fallback word extraction and the generic keyboard |
| Kelly frequency (8 languages) | ar el en it no ru sv zh | Top-1000 frequency list wired into suggestion ranking, on top of either tier |
|=========================================+

ONNX semantic models (opt-in, `--model`) resolve per the registry the
link:https://github.com/kotoshu/models-fasttext-onnx[models repo]
pins — 50+ languages across the tiers, in full/fluency/mini sizes.
...
Staged dictionaries for the other tiers carry their provenance in the
dictionaries repo manifest (`LICENSE` and source per file; LibreOffice
/ wooorm-dictionaries derivations dominate).
...
The YARD API reference for the Ruby API builds in CI on every change
to `lib/` (the `docs` workflow); the generated HTML is attached to each
run as the `yard-docs` artifact. A hosted copy is pending — it will
live either on this repository's GitHub Pages or as an API subpath on
https://kotoshu.github.io[kotoshu.github.io].

To build and browse it locally:

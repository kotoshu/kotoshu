  ...
  module Suggestions
    module Strategies
      ...
      class KeyboardProximityStrategy < BaseStrategy
        ...
        # Uses Damerau-Levenshtein distance (substitution, insertion,
        # deletion, adjacent transposition) via Algorithms::EditDistance,
        # so a swapped pair ("teh" -> "the") costs 1, not 2.
        #
        ...
        #
        # Each round applies one edit operation to every current
        # variant: substitution with a keyboard-adjacent key,
        # insertion of an adjacent key, deletion, and adjacent
        # transposition (the restricted Damerau operation — the most
        # common typing error, which is why it is enumerated here as a
        # single edit like the others).
        #
        ...
        def keyboard_variants(word, max_distance)
          ...
          max_distance.times do
            ...
            variants.each do |variant|
              ...
              # Adjacent transposition: swap each neighboring
              # character pair (a single 1-cost edit).
              (0...variant.length - 1).each do |i|
                swapped = variant[0...i] + variant[i + 1] + variant[i] + variant[(i + 2)..]
                new_variants.add(swapped)
              end
            end
